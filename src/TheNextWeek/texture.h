...
public:
	...
	virtual color value(double u, double v, const point3& p) const {
		//sin creates the marble-like effect, making the stripes undulate
		return color(1, 1, 1) * 0.5 * (1.0 + sin(scale * p.z() + 10 * noise.turb(p)));
	}

public:
	perlin noise;
	double scale;    //scale the coordinates to increase the frequency of the noise
};
